...
package object ItemByIdView {
  ...
  private type EditResponseT = HttpResponse.Ok[Unit] | ServerError |
    UnexpectedResponse
  ...
  def apply(pageS: Signal[ItemByIdPage]): ReactiveHtmlElement[HTMLDivElement] =
    ...
    val fetchItemRespS: Signal[Option[Item]] = fetchItemS.map {
      case Resolved(
            _,
    ...
    val submitS: EventStream[Status[
      (Int, ItemPut),
      Either[Throwable, EditResponseT]
    ]] =
      submitBus.events
          ...
          FetchStream
            .withDecoder(HttpResponse.handleServerErrorResponse orElse {
              case resp if resp.status == 200 =>
                EventStream.fromValue(HttpResponse.Ok(()))
    ...
    val submitStatusS: EventStream[(Boolean, String)] =
      submitS.map {
        ...
        case Resolved(_, Right(HttpResponse.Ok(_)), _) =>
    ...
    val submitSuccessS: EventStream[Unit] = submitS.collect {
      case Resolved(_, Right(HttpResponse.Ok(_)), _) => ()
    }

    div(
      pageS.mapTo(None) --> fetchedItemV.writer,
      h1(
      ...
      fetchItemRespS --> fetchedItemV.writer,
          ...
          onClick --> editedItemV
            .updater { (prev, _) =>
              prev match
                case Some(_) => None
                case None =>
                  fetchedItemV.now().map { item =>
                    (item.id, ItemDtoFlat.fromItem(item))
                  }
            }
        )
      ),
      ...
      child.maybe <-- fetchedItemV.signal
        ...
        .handleCase[
          ...
          ItemDisplay(signal).some
        }
        .handleCase[
          (Option[Item], Option[(Int, ItemDtoFlat)]),
          ItemDtoFlat,
          Option[ReactiveHtmlElement[HTMLElement]]
        ] { case (_, Some((_, item: ItemDtoFlat))) => item } { (_, signal) =>
          ItemEditForm(
            signal,
            editedItemV,
            fetchedItemV,
            submitBus,
            editedItemValidatedV
          ).some
        }
